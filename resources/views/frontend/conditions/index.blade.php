        ...
        <div class="col-md-12">
            @can('condition_create')
                <div style="margin-bottom: 10px;" class="row">
                    <div class="col-lg-12">
                        <a class="btn btn-success" href="{{ route('frontend.conditions.create') }}">
                            ...
                            <tbody>
                                @foreach($conditions as $key => $condition)
                                    <tr data-entry-id="{{ $condition->id }}">
                                        ...
                                        <td>
                                            @can('condition_show')
                                                <a class="btn btn-xs btn-primary" href="{{ route('frontend.conditions.show', $condition->id) }}">
                                                    {{ trans('global.view') }}
                                                </a>
                                            @endcan

                                            @can('condition_edit')
                                                <a class="btn btn-xs btn-info" href="{{ route('frontend.conditions.edit', $condition->id) }}">
                                                    {{ trans('global.edit') }}
                                                </a>
                                            @endcan

                                            @can('condition_delete')
                                                <form action="{{ route('frontend.conditions.destroy', $condition->id) }}" method="POST" onsubmit="return confirm('{{ trans('global.areYouSure') }}');" style="display: inline-block;">
...
@can('condition_delete')
  ...
  let deleteButton = {
    ...
    url: "{{ route('frontend.conditions.massDestroy') }}",
